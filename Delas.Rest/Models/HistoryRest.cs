...
{
    [DataContract]
    public partial class HistoryRest
    {
        public HistoryRest(int id, int idAccount, string title, double amount, string operationType, string destinationAccount, double balance, DateTime date)
        {
            this.Id = id;
        ...
        {
            Mapper.CreateMap<History, HistoryRest>()
            ...
            Mapper.CreateMap<HistoryRest, History>()
                .ForMember(d => d.Id, s => s.MapFrom(m => m.Id))
                .ForMember(d => d.IdAccount, s => s.MapFrom(m => m.IdAccount))
                .ForMember(d => d.Title, s => s.MapFrom(m => m.Title))
                .ForMember(d => d.Amount, s => s.MapFrom(m => m.Amount))
